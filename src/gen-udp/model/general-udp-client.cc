...
/*   if (m_type == 0) //Video
 ...
 if (m_type == 3) //Game Downlink
      {
    	  int m_nodeId = GetNode ()->GetId ();
    	 // ClientFile1 << "Voice" << "_" << m_nodeId << "_" <<  "_TX_time.csv";//commented by me
      }
*///this block is commented by me 
  if (m_socket == 0)
    {
...
GeneralUdpClient::Send (void)
{
  ...
  if (m_type == 0)
  {
	  ...
	  //ParetoRandomVariable pSize (326,1.2,250);//commented by me
//*******************Lines added by me **********

        Ptr<ParetoRandomVariable> pSize = CreateObject<ParetoRandomVariable> ();
       // pSize->SetAttribute ("Mean",  DoubleValue (326.0));
       // pSize->SetAttribute ("Shape", DoubleValue (1.2));
      //  pSize->SetAttribute ("bound", DoubleValue (250.0));//commented by me
//******************************************

	 m_size = pSize->GetInteger(20,10,0);//added by me, values randomly i have selected to avoid SIGIOT
       //  std::cout<<"For testing: added by me: m_size=  "<<m_size<<"\n";//adde by me for debugging
	  ...
	  if (m_sliceCount < 8)
	  {
		  //ParetoRandomVariable timeBwSlices (20.3 , 1.2 , 12.5);//commented by me
        //*******************Lines added by me **********

                Ptr<ParetoRandomVariable> timeBwSlices = CreateObject<ParetoRandomVariable> ();
               // timeBwSlices->SetAttribute ("Mean",  DoubleValue (20.3));
               // timeBwSlices->SetAttribute ("Shape", DoubleValue (1.2));
               //timeBwSlices->SetAttribute ("bound", DoubleValue (12.5));//commented by me
        //******************************************
		  m_sendEvent = Simulator::Schedule (MicroSeconds(1000*timeBwSlices->GetInteger(20.3,1.2,12.5)), &GeneralUdpClient::Send, this);
//std::cout<<"For testing: added by me: m_sendEvent=  "<<timeBwSlices->GetInteger(20,5,0)<<"\n";//added by me for debugging
	  }
	  else m_sliceCount=0;
...
//	  else m_sliceCount=0;
  ...
  else if (m_type == 1) //Gaming Uplink
  {
	  ...
	  {
		 // UniformVariable initialTime (0,40);commented by me
// Added by me
Ptr<UniformRandomVariable> initialTime = CreateObject<UniformRandomVariable> ();
initialTime->SetAttribute ("Min", DoubleValue (0));
initialTime->SetAttribute ("Max", DoubleValue (40));
  ...
  else if (m_type == 2) // 2 = Gaming Downlink
  {
	  if (m_sliceCount == 0) //First packet, delay uniformly [0, 40ms]
	  	  {
	  		  //UniformRandomVariable initialTime (0,40);
                        Ptr<UniformRandomVariable> initialTime= CreateObject<UniformRandomVariable> ();
                       initialTime->SetAttribute ("Min", DoubleValue (0));
                       initialTime->SetAttribute ("Max", DoubleValue (40));
	  		  m_sendEvent = Simulator::Schedule (MilliSeconds((uint32_t)initialTime->GetValue()), &GeneralUdpClient::Send, this);
  ...
  else if (m_type == 3) // 3 = VOIP
  {
	  if (m_voipActive) // VOIP active and talk frame initialized
	  {
		 ...
		 // UniformVariable z (0,1);//commented by me
