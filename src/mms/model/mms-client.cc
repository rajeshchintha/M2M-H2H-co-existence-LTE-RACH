...
MmsClient::Recv (Ptr<Socket> socket, int typeSpdu, Ptr<Packet> packet, Ptr<MmsServer> mmsServer, Address from)
{
	if (typeSpdu == AC) // The Server accept the connection, MMS Initiate Response received
	{
		...
		if ((m_typeRequesting == 0) || (m_typeRequesting == 1)) //type = 0 --> multicasting, type = 1 --> sequential
		{
			Ptr<Packet> packet = Create<Packet>(); // dummy packet
			Simulator::Schedule (Seconds (0.0), &MmsAdaptClient::ConfirmedRequest, m_mmsAdaptClient, socket, READ , mmsServer, packet, m_nReq);
		}
		if ((m_typeRequesting == 2) || (m_typeRequesting == 3)) //type = 2 --> report, type = 3 -->write
		{
			Ptr<Packet> packet = Create<Packet>(); // dummy packet
			Simulator::Schedule (Seconds (0.0), &MmsAdaptClient::ConfirmedRequest, m_mmsAdaptClient, socket, WRITE , mmsServer, packet, m_nReq);
		}
	}
	...
	if (typeSpdu == DTD)
	{
		...
		if (mmsTypeHeader.GetTypeMms() == CONFIRMED_RESPONSE)
		{
			InetSocketAddress address = InetSocketAddress::ConvertFrom (from);
			NS_LOG_INFO (Simulator::Now ().GetSeconds () << "s MMS-C ("<< m_SrcRef << ":" << Ipv4Address::ConvertFrom (m_localAddress) <<")"
				<< " has received CONFIRMED RESPONSE from MMS-S ("
				<< mmsServer->GetSrcRef () << ":"
				<< address.GetIpv4() << ")");
		...
		if ((m_typeRequesting == 0) || (m_typeRequesting == 3) )
		{
...
MmsClient::StartRequest ()
{
	...
	if ((m_typeRequesting == 0) || (m_typeRequesting == 2) || (m_typeRequesting == 3))
	{
		m_itSap = m_containerSap.Begin();
		while (m_itSap != m_containerSap.End ())
		{
...
//			t = Simulator::Now() + Seconds(0.001 * m_nSap);
	...
	if (m_typeRequesting == 1)
	{
		if (m_itSap == m_containerSap.Begin ())
		{
			...
			m_mmsAdaptClient->MmsAdapt(NULL, INITIATE, REQUEST, m_currentMmsServer, packet);
...
//		Simulator::Cancel (m_nextRequestEvent);
	...
	if (m_typeRequesting == 1)
	{
		...
		if (m_it_socket != m_activeSocket.end())
		{
			...
			Ptr<Packet> packet = Create<Packet>(); // dummy packet
			NS_LOG_INFO(m_nSap);
			Simulator::Schedule (Seconds (0.0), &MmsAdaptClient::ConfirmedRequest, m_mmsAdaptClient, socket, READ , m_currentMmsServer, packet, m_nReq);
			m_it ++;
			m_it_socket ++;
			m_nSap ++;
			NS_LOG_INFO (Simulator::Now ().GetSeconds () << "s MMS-C ("<< m_SrcRef << ":" << Ipv4Address::ConvertFrom (m_localAddress) <<")"
...
void MmsClient::StartInitiate(Ptr<MmsServer> mmsServer)
{
	...
	Simulator::Schedule(Seconds (0), &MmsAdaptClient::MmsAdapt, m_mmsAdaptClient, socket, INITIATE, REQUEST, mmsServer, packet);
}
