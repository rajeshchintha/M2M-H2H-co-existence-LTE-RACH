...
void
MmsAdaptClient::MmsAdapt(Ptr<Socket> socket, int mmsService, int typeService, Ptr<MmsServer> mmsServer, Ptr<Packet> packet)
{
	if (typeService == REQUEST)
	{
		if (mmsService == INITIATE)
		{
			NS_LOG_INFO (Simulator::Now().GetSeconds () << "s MMS-C has sent INITIATE-REQUEST to MMS-S ("
								<< mmsServer->GetSrcRef () << ":"
			...
			m_sendApduEvent = Simulator::Schedule (Seconds (0.0), &MmsAdaptClient::sendApdu, this, socket, packet, mmsServer);
		}
		if (mmsService == CONFIRMED)
		{
			m_sendApduEvent = Simulator::Schedule (Seconds (0.0), &MmsAdaptClient::sendConfirmedRequestApdu, this, socket, packet, mmsServer);
		}
	}
}

void
MmsAdaptClient::ConfirmedRequest (Ptr<Socket> socket, int mmsService, Ptr<MmsServer> mmsServer, Ptr<Packet> packet, uint8_t m_nReq)
{
	if (mmsService == READ)
	{
	...
	m_invokeMmsAdaptService = Simulator::Schedule (Seconds (0.0), &MmsAdaptClient::MmsAdapt, this, socket, CONFIRMED, REQUEST, mmsServer, packet);
...
void 
MmsAdaptClient::sendApdu (Ptr<Socket> socket, Ptr<Packet> packet,  Ptr<MmsServer> mmsServer)
{
//  NS_ASSERT (m_sendApduEvent.IsExpired ());
//  Simulator::Cancel (m_sendApduEvent);
  m_cotpClient->Send (socket, packet, CN, mmsServer);
}

void 
MmsAdaptClient::sendConfirmedRequestApdu (Ptr<Socket> socket, Ptr<Packet> packet,  Ptr<MmsServer> mmsServer)
{
  NS_ASSERT (m_sendApduEvent.IsExpired ());
  Simulator::Cancel (m_sendApduEvent);
  m_cotpClient->Send (socket, packet, DTD, mmsServer);
}
