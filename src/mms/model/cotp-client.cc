...
CotpClient::Recv (Ptr<Socket> socket)
{
  ...
  if (cotpTypeHeader.GetCotpPduType() == DT) // DT
  {
...
void
CotpClient::Send (Ptr<Socket> socket, Ptr<Packet> packet, int type, Ptr<MmsServer> mmsServer)
{ 
  // Retrieve the peer address
  m_remoteAddress = mmsServer->GetLocalAddress ();
