...
UeManager::SendRrcConnectionRelease ()
{
  ...
  std::map <uint8_t, Ptr<LteDataRadioBearerInfo> > all_bearers = this->GetRadioBearers();
  for (std::map <uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it = all_bearers.begin(); it != all_bearers.end(); ++it) {
      m_rrc->m_s1SapProvider->DoSendReleaseIndication (m_imsi, m_rnti, it->first);
  }
...
{
  NS_LOG_INFO("Sending release for bearer " << (uint32_t) bearerId);
...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
    ...
    .AddAttribute ("ConnectionRequestTimeoutDuration",
                   ...
                   TimeValue (MilliSeconds (100)), // this is not standard
    ...
    .AddAttribute ("ConnectionSetupTimeoutDuration",
                   ...
                   "SETUP COMPLETE.",
                   TimeValue (MilliSeconds (900)), // this is not standard, introduced to make up for RLF
...
LteEnbRrc::ConnectionRequestTimeout (uint16_t rnti)
{
  ...
  NS_LOG_INFO("Connection request timeout for Imsi " << GetUeManager (rnti)->GetImsi ());
...
{
  NS_LOG_FUNCTION (this << rnti);
  NS_LOG_INFO("Connection setup timeout for RNTI " << rnti);
...
LteEnbRrc::AddUe (UeManager::State state)
{
  ...
  NS_LOG_INFO (this << " New UE RNTI " << rnti << " cellId " << m_cellId << " srs CI " << ueManager->GetSrsConfigurationIndex ());
...
{
  NS_LOG_FUNCTION (this << " removing UE with RNTI " << rnti);
  ...
  m_connectionReleaseTrace(imsi, m_cellId, rnti);
}
...
LteEnbRrc::GetNewSrsConfigurationIndex ()
{
  ...
  NS_ASSERT (m_srsCurrentPeriodicityId > 0);
  NS_ASSERT (m_srsCurrentPeriodicityId < SRS_ENTRIES);
  NS_LOG_INFO (this << " SRS p " << g_srsPeriodicity[m_srsCurrentPeriodicityId] << " set " << m_ueSrsConfigurationIndexSet.size ());
  ...
  if (m_ueSrsConfigurationIndexSet.empty ())
  ...
  else
    {
      ...
      NS_LOG_INFO (this << " lower bound " << (*rit) << " of " << g_srsCiHigh[m_srsCurrentPeriodicityId]);
  ...
  NS_LOG_INFO("Set srs offset " << (uint32_t)m_lastAllocatedConfigurationIndex << " at time " << Simulator::Now().GetSeconds());
...
{
  NS_LOG_INFO("Release srs offeset " << (uint32_t) srcCi << " at time " << Simulator::Now().GetSeconds());
