...
public:
  ...
  void MmsAdapt(Ptr<Socket> socket, int mmsService, int typeService, Ptr<MmsServer> mmsServer, Ptr<Packet> packet);

  void ConfirmedRequest (Ptr<Socket> socket, int mmsService, Ptr<MmsServer> mmsServer, Ptr<Packet> packet, uint8_t m_nReq);

  void RecvSpdu (Ptr<Socket> socket, Ptr<Packet> packet, Address from);

  void sendApdu (Ptr<Socket> socket, Ptr<Packet> packet,  Ptr<MmsServer> mmsServer);

  void sendConfirmedRequestApdu (Ptr<Socket> socket, Ptr<Packet> packet,  Ptr<MmsServer> mmsServer);
